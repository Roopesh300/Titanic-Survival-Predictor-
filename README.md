...
```
.
├── RoopeshRajuSV_TitanicSurvivalPredictor.ipynb   ← Jupyter Notebook (full pipeline)
├── app.py                                          ← Standalone Python script
├── requirements.txt                               ← Python dependencies
├── RoopeshRajuSV_ProjectReport.docx               ← Full project report (Word)
├── README.md                                      ← This file
...
### Step 2 — Option A: Run the Python Script

```bash
python app.py
```

### Step 2 — Option B: Open the Jupyter Notebook

```bash
jupyter notebook RoopeshRajuSV_TitanicSurvivalPredictor.ipynb
```

Then run all cells from top to bottom. The last cell launches the Gradio UI.
...
> The CSV file (`Titanic-Dataset.csv`) must be placed in the same directory as `app.py` or the notebook.
